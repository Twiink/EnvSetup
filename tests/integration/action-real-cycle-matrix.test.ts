...
async function isScoopGitInstalled(): Promise<boolean> {
  return commandSucceeds('powershell', [
    ...
    'Bypass',
    '-Command',
    `$scoop = $null; $candidate = Join-Path $env:USERPROFILE 'scoop\\shims\\scoop.cmd'; if (Test-Path $candidate) { $scoop = $candidate }; if (-not $scoop) { $scoop = (Get-Command 'scoop.cmd' -ErrorAction SilentlyContinue).Source }; if (-not $scoop) { $scoop = (Get-Command 'scoop' -ErrorAction SilentlyContinue).Source }; if (-not $scoop) { exit 1 }; & $scoop prefix git *> $null; if ($LASTEXITCODE -eq 0) { exit 0 }; $roots = @(); $shimDir = Split-Path $scoop -Parent; $roots += Split-Path $shimDir -Parent; if ($env:SCOOP) { $roots += $env:SCOOP }; $roots += Join-Path $env:USERPROFILE 'scoop'; $roots = $roots | Select-Object -Unique; foreach ($r in $roots) { $gc = Join-Path $r 'apps\\git\\current'; if (Test-Path $gc) { exit 0 }; $gd = Join-Path $r 'apps\\git'; if (Test-Path $gd) { $vd = Get-ChildItem -Path $gd -Directory -ErrorAction SilentlyContinue | Where-Object { $_.Name -ne 'current' } | Select-Object -First 1; if ($vd) { exit 0 } } }; exit 1`,
  ])
}
