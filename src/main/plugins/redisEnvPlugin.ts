...
function buildMemuraiCleanupCommand(installDir?: string): string {
  ...
  return [
    ...
    'if ($command -match \'\\{[A-Za-z0-9\\-]+\\}\') { $productCode = $matches[0]; & msiexec.exe /x $productCode /quiet /norestart; $uninstallExitCode = $LASTEXITCODE; if ($uninstallExitCode -ne 0 -and $uninstallExitCode -ne 1641 -and $uninstallExitCode -ne 3010) { throw "Memurai uninstall failed with exit code $($uninstallExitCode)." } }',
    '}',
    removeInstallDir,
  ]
    .filter(Boolean)
